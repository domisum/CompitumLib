...
import de.domisum.lib.compitum.navgraph.json.SerializationGraph;
import de.domisum.lib.compitum.navgraph.edit.NavGraphEditManager;
...
public class NavGraphManager
{
	...
	{
		CompitumLib.getLogger().info("Loading NavGraphs...");
		...
		for(File file : FileUtil.listFilesRecursively(baseDir))
		{
			...
			try
			{
				...
				CompitumLib.getLogger().info("Loaded NavGraph '"+graph.getId()+"'");
			}
			catch(Exception e)
			{
				e.printStackTrace();
		...
		CompitumLib.getLogger().info("Loading NavGraphs complete: loaded "+this.graphs.size()+" NavGraph(s)");
	}
	...
	{
		CompitumLib.getLogger().info("Saving NavGraphs...");
		...
		CompitumLib.getLogger().info("Saving NavGraphs complete: saved "+this.graphs.size()+" NavGraph(s)");
	}
